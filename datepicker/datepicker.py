...
class DatePicker(ft.UserControl):
    ...
    def _create_calendar(self, year, month, hour, minute, hide_ymhm=False):
        ...
        for w in range(0, weeks_rows_num):
            ...
            for d in days[w]:
                ...
                d = datetime(d.year, d.month, d.day, self.hour, self.minute) if self.hour_minute else datetime(d.year,
                                                                                                               d.month,
                                                                                                               d.day)
    ...
    def _year_month_selectors(self, year, month, hide_ymhm=False):
        prev_year = ft.IconButton(icon=ft.icons.ARROW_BACK, data=self.PREV_YEAR,
                                  on_click=self._adjust_calendar) if not hide_ymhm else ft.Text(self.EMPTY,
                                                                                                height=self.CELL_SIZE, )
        next_year = ft.IconButton(icon=ft.icons.ARROW_FORWARD, data=self.NEXT_YEAR,
                                  on_click=self._adjust_calendar) if not hide_ymhm else ft.Text(self.EMPTY)
        prev_month = ft.IconButton(icon=ft.icons.ARROW_BACK, data=self.PREV_MONTH,
                                   on_click=self._adjust_calendar) if not hide_ymhm else ft.Text(self.EMPTY)
        next_month = ft.IconButton(icon=ft.icons.ARROW_FORWARD, data=self.NEXT_MONTH,
                                   on_click=self._adjust_calendar) if not hide_ymhm else ft.Text(self.EMPTY)
        ym = ft.Row([
            ft.Row([
                prev_year,
